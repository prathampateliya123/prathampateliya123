"""
Render "PRATHAM" as an EXTRUDED 3D wordmark rasterized to ASCII, and emit it as an
...
# ---- geometry / grid ------------------------------------------------------
# Futura Bold on macOS; fall back to a bold Windows face so local + CI both work.
# Even stroke weight keeps shading consistent; a very heavy face (Impact) collapses
# counters at this resolution.
_DEFAULT_FONTS = [
...
if not FONT_PATH:
    for _candidate in _DEFAULT_FONTS:
        if os.path.exists(_candidate):
            FONT_PATH = _candidate
            break
    else:
        FONT_PATH = _DEFAULT_FONTS[0]
FONT_INDEX = int(os.environ.get("WORDMARK_FONT_INDEX", "2" if FONT_PATH.endswith(".ttc") else "0"))
# PRATHAM is 7 letters — widen the grid so each glyph still lands ~12–14 cols wide
# (AVI used 50 cols / 3 letters). Wider SVG is fine; README width=490 scales it down.
TEXT = os.environ.get("WORDMARK_TEXT", "PRATHAM")
COLS = int(os.environ.get("WORDMARK_COLS", "78"))
ROWS = 0               # derived from the art -- see fit()
# blank rows above and below the art. Keep the panel height matched to the portrait
# when README shows portrait @370px and wordmark @490px.
ROW_MARGIN = int(os.environ.get("WORDMARK_ROW_MARGIN", "14"))
CELL_W = 9.0
CELL_H = 15.5
MASK_H = 300           # glyph raster height in mask px (drives voxel density)
# slightly tighter tracking so 7 letters stay one readable slab without melting together.
TRACKING = float(os.environ.get("WORDMARK_TRACKING", "0.08"))
LINE_GAP = 1.20        # baseline-to-baseline, in cap heights (multi-line TEXT only)
DEPTH_FRAC = 0.30      # extrusion depth as a fraction of glyph height
TILT_DEG = float(os.environ.get("WORDMARK_TILT", "3.0"))
                       # fixed X tilt so the top face stays visible.
CAM_DIST = 7.2         # camera distance in world units (1.0 == wordmark width)
FOCAL = 4.6
...
def emit(frames, mode, out, dur, reveal):
    ...
    p.append(f'<text x="{canvas_w/2:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" fill="{TITLE_TEXT}" '
             f'font-size="11.5" text-anchor="middle">pratham@github: ~$ ./wordmark.sh --3d</text>')
